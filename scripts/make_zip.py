...
def  main():
  ...
  installer = "\installer\SVGUITest Installer.exe"
   
  if demo:
    installer = "\installer\SVGUITest Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SVGUITest manual.pdf" 
  ]
  ...
  ZIPNAME = "SVGUITest-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SVGUITest-v" + FULLVERSIONSTR + "-win-demo.zip"
